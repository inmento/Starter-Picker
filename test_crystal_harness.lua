...
local mod = {
  ...
  content = {
    ...
    map_scripts = { register = function() error("Crystal must not register Gen 1 map scripts") end },
  },
  options = {
...
assert(#callbacks.schema == 10, "expanded Crystal starter option schema was not registered")
assert(callbacks.schema[1].key == "cyndaquil_ball", "Crystal left-ball selector missing")
assert(callbacks.schema[4].key == "starter_trade_evolution_42", "shared trade fallback option missing")
assert(callbacks.schema[9].key == "gold_player_dv_mode", "Crystal player-DV mode missing")
assert(callbacks.schema[10].key == "gold_held_item", "Crystal held-item selector missing")
...
for index, label in ipairs(expectedLabels) do
  assert(callbacks.schema[index].label == label,
    "Crystal mobile option label drifted at row " .. index)
  assert(#label <= 10, "Crystal mobile option label exceeds the safe 10-character width: " .. label)
end
...
callbacks.hooks["script.command"](function(_, _, _, finalCmd)
  ...
  return finalCmd
end, { generation = 2, scriptKey = "1e:4c73" }, "pokepic", {}, { species = 155 })
callbacks.hooks["script.command"](function(_, _, _, finalCmd)
  previewCry = finalCmd
  return finalCmd
end, { generation = 2, scriptKey = "1e:4c73" }, "cry", {}, { id = 155 })
assert(previewPicture and previewPicture.species == 251,
  "Crystal Elm preview portrait did not use the configured left-ball species")
assert(previewCry and previewCry.id == 251,
  "Crystal Elm preview cry did not use the configured left-ball species")

local dispatched
callbacks.hooks["script.command"](function(_, _, _, finalCmd)
  dispatched = finalCmd
  local starterId = assert(byIndex[finalCmd.species], "rewritten Crystal starter index was invalid")
  table.insert(game.save.party,
    { species = starterId, level = finalCmd.level, item = finalCmd.item,
      ...
      stats = { hp = 20 }, statExp = {} })
end, { generation = 2, scriptKey = "1e:4c73" }, "givepoke", {},
  { species = 151, level = 5, item = 173 })
assert(dispatched and dispatched.species == 151, "Crystal accepted starter did not retain the transformed species")
assert(dispatched.item == 139, "Crystal accepted starter did not use configured held item")
local player = game.save.party[1]
assert(player.species == "MEW" and player.name == "MEW", "Crystal player starter was not rebuilt correctly")
assert(player.item == "BERRY_JUICE", "Crystal player starter held item was not preserved as the selected ID")
assert(player.dvs.attack == 15 and player.dvs.defense == 15
  and player.dvs.speed == 15 and player.dvs.special == 15,
  "Crystal maximum player starter DVs were not applied")
assert(game.save.pokedex.seen.MEW and game.save.pokedex.caught.MEW,
  "Crystal synchronized starter did not update Pokédex ownership")
assert(optionValues.cyndaquil_ball == "MEW",
  "Crystal Randomizer-transformed starter was not copied into Starter Picker options")
assert(storage.finalized_starter_selection and storage.finalized_starter_selection.species == "MEW",
  "Crystal finalized starter record was not stored")
...
assert(openedText and openedText:find("CONFIRMED LEFT", 1, true),
  "Crystal starter status did not report the finalized selection")
callbacks.events["mod.options_changed"]({ mod="starter_picker", key="rival_preview", value=true })
assert(openedText and openedText:find("RIVAL", 1, true), "Crystal rival preview did not open")
...
assert(game.save.party[1].item == "BERRY" or game.save.party[1].item == "BERRY_JUICE",
  "Crystal recovery held-item mode did not choose a safe recovery item")
optionValues.gold_held_item = "NONE"
callbacks.events["mod.options_changed"]({ mod="starter_picker", key="gold_held_item", value="NONE" })
assert(game.save.party[1].item == nil, "Crystal no-item starter mode did not remove the held item")
...
assert(projected[#projected].species == "CHIKORITA",
  "Crystal rival did not choose the weakness-aware counter starter")
assert(rivalParty[#rivalParty].species == "CYNDAQUIL",
  "Crystal rival projection mutated the vanilla party")

print("crystal starter picker randomizer-sync, preview-cry, accepted-starter, held-item, DV, and rival harness: valid")
